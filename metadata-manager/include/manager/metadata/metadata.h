...
class Metadata {
 public:
  ...
  /**
   *  @brief  Loads the metadata which specific generation from
   * metadata-table.
 ...
 protected:
   ...
   *  @return ErrorCode::OK if success, otherwise an error code.
   */
  static ErrorCode load(std::string_view database, std::string_view tablename,
   ...
   *  @param  (database)   [in]  database name.
   *  @param  (tablename)  [in]  metadata-table name.
   *  @param  (pt)         [in]  property_tree object that stores metadata to
   * be saved.
   *  @param  (generation) [out] the generation of saved metadata.
   */
  static ErrorCode save(std::string_view database, std::string_view tablename,
                        boost::property_tree::ptree &pt,
