...
void ApiTestTableMetadata::check_metadata_expected(ptree &expected,
    ...
    boost::optional<ptree &> o_expected =
        expected.get_child_optional(meta_name);
...
TEST_F(ApiTestTableMetadata, get_two_table_metadata_by_table_name) {
    ...
    // verifies that the returned table metadata is expected one.
    int check_count = 0;
    BOOST_FOREACH (ptree::value_type &node, table_metadata_inserted) {
        ptree &table = node.second;
        new_table.put(Tables::ID, ret_table_id[check_count]);
        ApiTestTableMetadata::check_table_metadata_expected(new_table, table);
        check_count++;
    }
}
...
TEST_F(ApiTestTableMetadata,
    ...
    error = tables_get_by_name->get(new_table_name,
