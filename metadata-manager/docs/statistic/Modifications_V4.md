...
- [統合メタデータ管理基盤 統計情報管理機能 改造仕様書](#%E7%B5%B1%E5%90%88%E3%83%A1%E3%82%BF%E3%83%87%E3%83%BC%E3%82%BF%E7%AE%A1%E7%90%86%E5%9F%BA%E7%9B%A4-%E7%B5%B1%E8%A8%88%E6%83%85%E5%A0%B1%E7%AE%A1%E7%90%86%E6%A9%9F%E8%83%BD-%E6%94%B9%E9%80%A0%E4%BB%95%E6%A7%98%E6%9B%B8)
  ...
  - [付録](#%E4%BB%98%E9%8C%B2)
    ...
    - [F. 案１の統計情報オブジェクト](#f-%E6%A1%88%EF%BC%91%E3%81%AE%E7%B5%B1%E8%A8%88%E6%83%85%E5%A0%B1%E3%82%AA%E3%83%96%E3%82%B8%E3%82%A7%E3%82%AF%E3%83%88)
    ...
    - [G. 案２の統計情報オブジェクト](#g-%E6%A1%88%EF%BC%92%E3%81%AE%E7%B5%B1%E8%A8%88%E6%83%85%E5%A0%B1%E3%82%AA%E3%83%96%E3%82%B8%E3%82%A7%E3%82%AF%E3%83%88)
...
- 案１：統計情報クラス(Statistics)を表統計情報クラス(TableStatistics)と列統計情報クラス(ColumnStatistics)に分ける
- 案２：表統計情報と列統計情報を1つの統計情報クラスにまとめる
- 案３：統計情報クラス(Statistics)では列統計情報クラスのみ扱い、表統計情報はTableクラス(Tables)で扱う

案１では、既存のStatisticsクラスを廃止し、表統計情報クラスと列統計情報クラスに分ける。この案では、1つの表に関する統計情報（表統計情報と列統計情報）を取得するために最低でも２回のメソッド呼び出し（DBアクセス）が必要となる（現バージョンと同じ）。
...
案３では、Statisticsクラスで扱う統計情報を列統計情報のみとし、表統計情報はTableクラスで扱うように変更する。現状、表統計情報は行数情報のみのため簡便性を優先してテーブルメタデータに含める。将来、表統計情報の種類が増えてきた場合は、案１か案２を検討する。Tableクラスにはあらたに表統計情報を扱うメソッドを追加する。この案では、1つの表に関する統計情報（表統計情報と列統計情報）を取得するために最低でも２回のメソッド呼び出し（DBアクセス）が必要となる。

どの案であっても大きな違いはないと考えるが、設計をシンプルにするために今回は案３を選択する。
...
- 例1-1）addメソッド実行時
  ...
  ```json
  {
    "name": "PUBLIC.foo", "namespace": "PUBLIC", "primary_keys": {[1,2]}, "tuples": 231000, 
    "columns": [{...}, {...}, {...}]
  }
  ```
...
- 例1-2）getメソッド実行時
  ...
  ```json
  {
    "format_version": 1, "generation": 1, "id": 3, "name": "PUBLIC.foo", "namespace": "PUBLIC", 
    "primary_keys": {[1,2]}, "tuples": 231000, 
    "columns": [{...}, {...}, {...}]
  }
  ```
...
- 例2-2）getメソッド実行時
  ...
  ```json
  {
    "format_version": 1, "generation": 1, "id": 3, "name": "PUBLIC.foo", "namespace": "", 
    "primary_keys": {[]}, "tuples": 0, 
    "columns": [{...}, {...}, {...}]
  }
  ```
...
- 変更後のgetメソッド
  ...
  - Tablesクラス
    - get(id, ptree);
    - get(name, ptree);
    - get_all(objects) // すべてのテーブルオブジェクトを取得する
    - get_stat(id, ptree);
    - get_stat(name, ptree)
  ...
  - Statisticsクラス
    ...
    - get_from_column_id(table_id, column_id, ptree);
    - get_from_column_name(table_id, column_name, ptree);
    - get_from_column_number(table_id, column_number, ptree);
...
| 6.| ordinal_position  | bigint    |- |- |-  | カラム番号。                 |1 origin。|
| 7.| column_id         | bigint    |- |- |-  | カラムのOID。                |-|
| 8.| column_name       | string    |- |- |-  | カラム名。                   |-|
| 9.| column_statistic  | json      |- |- |-  | カラム統計情報。             |-|
