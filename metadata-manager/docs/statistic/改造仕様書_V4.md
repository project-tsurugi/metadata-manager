...
- 変更後のgetメソッド
  ...
  - Statisticsクラス
    ...
    - get_from_column_id(table_id, column_id, ptree); // カラムIDを指定して列統計情報を取得する
    - get_from_column_name(table_id, column_name, ptree); // カラム名を指定して列統計情報を取得する
    - get_from_column_number(table_id, column_number, ptree); // カラム番号を指定して列統計情報を取得する
