...
* 事後条件
  ...
  * 戻り値として、ErrorCodeを返す。
    ...
    * ErrorCode::DATABASE_ACCESS_FAILUREが返ってきた場合
      ...
      * メタデータ管理機能APIを利用すると、ErrorCode::NOT_INITIALIZEDが返される。
...
|NOT_INITIALIZED|次の場合、このエラーコードが返される。<br/>・初期化処理に失敗してmetadata-managerのAPIを利用した場合<br/>・初期化処理を行わずにmetadata-managerのAPIを利用した場合|
