...
* 本APIを呼んでErrorCode::OK以外のエラーコードが返されたあとでmetadata-managerのAPIを利用した場合、または、本APIを呼ばずにmetadata-managerのAPIを利用した場合、このErrorCode::NOT_INITIALIZEDが返される。
...
| プロパティ名（論理名） | 型        | 値の範囲                                                     |
| ---------------------- | --------- | ------------------------------------------------------------ |
| エラーコード           | ErrorCode | ErrorCode::OK：正常<br />ErrorCode::NOT_INITIALIZED：メタデータ格納先に障害が発生した。|
...
* 事後条件
  ...
  * 戻り値として、ErrorCodeを返す。
    ...
    * ErrorCode::NOT_INITIALIZEDが返ってきた場合
      * 列統計・表統計管理機能 APIを利用すると、ErrorCode::NOT_INITIALIZEDが返される。
      * メタデータ格納先とメタデータの受け渡しができない。
...
|---|---|
|OK|正常|
|NOT_INITIALIZED|次の場合、このエラーコードが返される。<br/>・初期化処理（コネクションの確立など）に失敗した場合<br/>・初期化処理に失敗してmetadata-managerのAPIを利用した場合<br/>・初期化処理を行わずにmetadata-managerのAPIを利用した場合|
|INVALID_PARAMETER|引数に不正なパラメーターが入力された。metadata-managerのAPIを利用するコンポーネントに問題あり。<br/>・UPDATE/INSERT/UPSERT文の発行に失敗：メタデータ格納先のテーブルに対して、制約に違反するレコードの登録・更新を行った。<br/>存在しないテーブルID（またはテーブル名）またはカラム番号のレコードの更新を行った。<br/>・DELETE文の発行に失敗：メタデータ格納先のテーブルに対して、存在しないテーブルID（またはテーブル名）またはカラム番号のレコードの削除を行った。<br/>・SELECT文の発行に失敗：メタデータ格納先のテーブルに対して、存在しないテーブルID（またはテーブル名）またはカラム番号のレコードの参照を行った。<br/>・または、メタデータ格納先に障害が発生した。|
|DATABASE_ACCESS_FAILURE|メタデータ格納先に障害が発生した。メタデータ格納先（データベース）周りに問題あり。<br/>・コネクションが確立できない<br/>・セキュアなサーチパスの設定に失敗<br/>・プリペアードステートメントの前処理の実行に失敗<br/>・トランザクションの開始に失敗<br/>・コミット失敗<br/>・ロールバック失敗|
