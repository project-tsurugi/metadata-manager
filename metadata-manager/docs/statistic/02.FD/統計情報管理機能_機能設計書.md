...
- [metadata-manager 列統計・表統計 管理機能 機能設計](#metadata-manager-列統計・表統計-管理機能-機能設計)
    - [目次](#目次)
    - [用語](#用語)
    - [列統計・表統計 管理機能 概要](#列統計・表統計-管理機能-概要)
        - [2021年3月までの開発スコープ](#2021年3月までの開発スコープ)
        - [列統計・表統計 管理機能APIのinput](#列統計・表統計-管理機能apiのinput)
    - [メタデータ格納先のデータベース設計](#メタデータ格納先のデータベース設計)
        - [概要](#概要)
        - [メタデータ格納先](#メタデータ格納先)
        - [メタデータ格納先テーブルのデータベース・スキーマ](#メタデータ格納先テーブルのデータベース・スキーマ)
            - [理由](#理由)
    ...
    - [メタデータ管理機能 共通API](#メタデータ管理機能-共通api)
        ...
        - [切断処理](#切断処理)
            ...
            - [戻り値](#戻り値-1)
            - [条件](#条件-1)
    - [列統計・表統計管理機能 API](#列統計・表統計管理機能-api)
...
### 列統計・表統計 管理機能APIのinput

* 列統計・表統計 管理機能APIのinputを「テーブルID」「カラム番号」とした理由

  1. 各テーブルの「テーブルID」、「カラム数またはカラム番号のリスト」をメモリに保持しておけば、「テーブルID」「カラム番号」をinputとして、列統計・表統計管理APIを利用できるため。
     * カラムIDやカラム名ではなく、カラム番号をinputとすることで、Columnメタデータテーブルにアクセスする必要がなくなるため。
     * 列統計・表統計管理機能API利用の流れ
       1. metadata-managerのAPIを利用するTsurugiのコンポーネントは、次の処理を行うときに、各テーブルの「テーブルID」、「カラム数またはカラム番号のリスト」をメモリに保持する。
          * Tsurugiの利用者がDDL(CREATE TABLE,ALTER TABLE)を発行したとき
          * 自コンポーネントが起動するとき
            * 自コンポーネントが起動するときは、Tableメタデータテーブルから「テーブルID」、「カラム数またはカラム番号のリスト」を取得する。
  2. metadata-managerのAPIを利用するTsurugiのコンポーネント（ogawayamaなど）は、列統計・表統計管理機能APIを利用する。
  2. metadata-managerのAPIを利用するTsurugiのコンポーネント間で、列統計・表統計を交換するとき、テーブルID・カラム番号を指定して交換するため。
...
| プロパティ名（論理名） | 型        | 値の範囲                                                     |
| ---------------------- | --------- | ------------------------------------------------------------ |
| エラーコード           | ErrorCode | ErrorCode::OK：正常<br />ErrorCode::OPEN_CONNECTION_FAILURE：コネクションの確立に失敗<br/>ErrorCode::SET_ALWAYS_SECURE_SEARCH_PATH_FAILURE：セキュアなサーチパスの設定に失敗<br/>ErrorCode::PREPARE_STATEMENT_FAILURE：SQL文の前処理に失敗 |
...
| プロパティ名（論理名） | 型        | 値の範囲                                                     |
| ---------------------- | --------- | ------------------------------------------------------------ |
| エラーコード           | ErrorCode | ErrorCode::OK：正常<br />ErrorCode::CLOSE_CONNECTION_FAILURE：コネクションの切断に失敗 |
...
* 前提
  * Tsurugiのインストール時に、[Tsurugi利用者が実施する環境設定](#Tsurugi利用者が実施する環境設定)が実施済みである。
  * 各テーブルの「テーブルID」、「カラム数またはカラム番号のリスト」をメモリに保持している。
...
| ---------------------------- | ---------------------------- | ---------------- | ------------ | ------ | -------- | -------- |
| テーブルID                   | table_id                   | int64_t |              | IN     | 省略不可 | 1桁以上  |
| カラム番号                     | ordinal_position                  | int64_t |              | IN     | 省略不可 | 1桁以上  |
...
| プロパティ名（論理名） | 型        | 値の範囲                                                     |
| ---------------------- | --------- | ------------------------------------------------------------ |
| エラーコード           | ErrorCode | ErrorCode::OK：正常<br />ErrorCode::INVALID_PARAMETER：引数に不正なパラメーターが入力された<br/>ErrorCode::UPSERT_FAILURE：UPSERTのSQL文の発行に失敗<br/>ErrorCode::BEGIN_TRANSACTION_FAILURE：トランザクションの開始に失敗<br/>ErrorCode::COMMIT_FAILURE：コミット失敗<br/>ErrorCode::ROLLBACK_FAILURE：ロールバック失敗 |
...
5. 3.でレコードの登録・更新に失敗した場合
   ...
   2. metadata-managerは、戻り値にErrorCode::UPSERT_FAILUREを設定し、処理を終了する。
...
| プロパティ名（論理名） | 型        | 値の範囲                                                     |
| ---------------------- | --------- | ------------------------------------------------------------ |
| エラーコード           | ErrorCode | ErrorCode::OK：正常<br />ErrorCode::INVALID_PARAMETER：引数に不正なパラメーターが入力された<br/>ErrorCode::UPDATE_FAILURE：UPDATEのSQL文の発行に失敗<br/>ErrorCode::BEGIN_TRANSACTION_FAILURE：トランザクションの開始に失敗<br/>ErrorCode::COMMIT_FAILURE：コミット失敗<br/>ErrorCode::ROLLBACK_FAILURE：ロールバック失敗|
...
5. 3.でレコードの登録・更新に失敗した場合
   ...
   2. metadata-managerは、戻り値にErrorCode::UPDATE_FAILUREを設定し、処理を終了する。
...
| プロパティ名（論理名） | 型        | 値の範囲                                           |
| ---------------------- | --------- | -------------------------------------------------- |
| エラーコード           | ErrorCode | ErrorCode::OK：正常<br />ErrorCode::INVALID_PARAMETER：引数に不正なパラメーターが入力された<br/>ErrorCode::SELECT_FAILURE：SELECTのSQL文の発行に失敗 |
...
2. metadata-managerは、メタデータ格納先の列統計テーブル【tsurugi_statistic】から、テーブルIDが引数テーブルIDに一致する全レコードを取得する。
   ...
   * それ以外の場合
     ...
     * metadata-managerは、戻り値にErrorCode::SELECT_FAILUREを設定し、処理を終了する。
...
| --------------------------- | ---------------------- | ------------------ | ------------ | ------ | -------- | -------- |
| テーブルID                  | table_id             | int64_t   |              | IN     | 省略不可 | 1桁以上  |
| カラム番号(1 origin)                    | ordinal_position           | int64_t   |              | IN     | 省略不可 | 1桁以上  |
...
| プロパティ名（論理名） | 型        | 値の範囲                                                     |
| ---------------------- | --------- | ------------------------------------------------------------ |
| エラーコード           | ErrorCode | ErrorCode::OK：正常<br />ErrorCode::INVALID_PARAMETER：引数に不正なパラメーターが入力された<br/>ErrorCode::DELETE_FAILURE：DELETEのSQL文の発行に失敗<br/>ErrorCode::BEGIN_TRANSACTION_FAILURE：トランザクションの開始に失敗<br/>ErrorCode::COMMIT_FAILURE：コミット失敗<br/>ErrorCode::ROLLBACK_FAILURE：ロールバック失敗|
...
5. 3.でレコードの削除に失敗した場合
   1. メタデータ格納先は、rollback処理を行い、Transactionを終了する。
   2. metadata-managerは、戻り値にErrorCode::DELETE_FAILUREを設定し、処理を終了する。
...
|---|---|
|OK|正常|
|INVALID_PARAMETER|引数に不正なパラメーターが入力された|
|OPEN_CONNECTION_FAILURE|メタデータ格納先とのコネクション確立失敗|
|CLOSE_CONNECTION_FAILURE|メタデータ格納先とのコネクション切断失敗|
|SET_ALWAYS_SECURE_SEARCH_PATH_FAILURE|セキュアなサーチパスの設定に失敗|
|PREPARE_STATEMENT_FAILURE|プリペアードステートメントの前処理の実行に失敗|
|BEGIN_TRANSACTION_FAILURE|トランザクションの開始に失敗|
|COMMIT_FAILURE|コミット失敗|
|ROLLBACK_FAILURE|ロールバック失敗|
|UPSERT_FAILURE|UPSERTのSQL文の発行に失敗・ロールバック成功|
|UPDATE_FAILURE|UPDATEのSQL文の発行に失敗・ロールバック成功|
|SELECT_FAILURE|SELECTのSQL文の発行に失敗|
|DELETE_FAILURE|DELETEのSQL文の発行に失敗・ロールバック成功|
