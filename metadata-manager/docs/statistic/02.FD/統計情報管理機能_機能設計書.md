...
* 本APIを呼んでErrorCode::OK以外のエラーコードが返されたあとでmetadata-managerのAPIを利用した場合、または、本APIを呼ばずにmetadata-managerのAPIを利用した場合、このErrorCode::NO_CONNECTIONが返される。
...
* 事後条件
  ...
  * 戻り値として、ErrorCodeを返す。
    ...
    * ErrorCode::DATABASE_ACCESS_FAILUREが返ってきた場合
      * 列統計・表統計管理機能 APIを利用すると、ErrorCode::NO_CONNECTIONが返される。
...
|---|---|
|OK|正常|
|NO_CONNECTION|初期化処理でErrorCode::OK以外のエラーコードが返されたあとで、または、<br/>初期化処理（コネクションの確立・セキュアなサーチパスの設定・プリペアードステートメントの前処理）を行っていないあとで、<br/>metadata-managerのAPIを利用した場合、このエラーコードが返される。|
