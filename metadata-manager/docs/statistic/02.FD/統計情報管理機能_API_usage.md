...
| メソッド名   | 説明                                                         |
| ----------- | ------------------------------------------------------------ |
|connect()|・下記3つの処理を実施し、すべての処理が成功した場合、metadata-managerは、メタデータ格納先とメタデータの受け渡しが可能となる。<br />1. metadata-managerは、メタデータ格納先とのコネクションを確立する。<br/>2. サーチパスがセキュアになるように設定する。<br/>metadata-managerは、メタデータ格納先にプリペアードステートメントを送り、SQL文の前処理を実行する。<br/>3. <br>・本クラスのインスタンスごとに1つのコネクションが確立される。<br>・子クラスのメソッドで実装を行う。|
...
* metadata-managerのAPIを利用するTsurugiのコンポーネント起動時
  ...
  //2. metadata-managerは、メタデータ格納先にプリペアードステートメントを送り、SQL文の前処理を実行する。
  // 上記2つすべてが成功した場合、metadata-managerは、メタデータ格納先とメタデータの受け渡しが可能となる。
...
* 列統計の削除
  ...
  * 1カラムの列統計削除
    ...
    if (ErrorCode::OK != stats->removeAllColumnStatistics(table_id, column_ordinal_position))
    {
    	// error handling
    }
    ```
