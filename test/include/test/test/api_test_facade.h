...
class ApiTestFacade : protected ApiTest, public ::testing::Test {
 public:
  ...
  OBJECT* get_structure() const override {
    OBJECT* res_object = nullptr;
    if (!std::is_same<OBJECT, DummyObject>::value) {
      res_object = metadata_struct_.get();
    }
    return res_object;
  }
   ...
   */
  void test_flow_getall(const UtMetadata<OBJECT>& ut_metadata,
