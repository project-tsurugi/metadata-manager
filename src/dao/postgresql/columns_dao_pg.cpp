...
ErrorCode ColumnsDaoPg::insert(const boost::property_tree::ptree& object,
  ...
  params.emplace_back((data_type_id.empty() ? nullptr : data_type_id.c_str()));
  ...
  params.emplace_back((!data_length_json.empty() ? data_length_json.c_str()
                                                 : kEmptyStringJson));
  ...
  params.emplace_back(default_expr.empty() ? nullptr : default_expr.c_str());

  // is_funcexpr
  auto is_funcexpr = ptree_helper::ptree_value_to_string<std::string>(
      object, Column::IS_FUNCEXPR);
  params.emplace_back(is_funcexpr.empty() ? nullptr : is_funcexpr.c_str());
...
void ColumnsDaoPg::create_prepared_statements() {
...

  {
    ...
    SelectStatement statement_name{
        ...
        this->get_select_statement(ColumnName::kTableId), Column::TABLE_ID};
...

  {
    ...
    DeleteStatement statement_name{
        this->get_source_name(),
        this->get_delete_statement(ColumnName::kTableId), Column::TABLE_ID};
...
boost::property_tree::ptree ColumnsDaoPg::convert_pgresult_to_ptree(
  ...
  object.put(Column::IS_FUNCEXPR,
             get_result_value<bool>(pg_result, row_number,
                                    OrdinalPosition::kIsFuncExpr));
