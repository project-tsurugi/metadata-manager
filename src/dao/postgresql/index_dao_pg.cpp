...
ErrorCode IndexDaoPg::convert_pgresult_to_ptree(
  ...
  object.put(Object::ID, PQgetvalue(res, row_number,
  ...
  object.put(Index::ACL, PQgetvalue(res, row_number,
  ...
  object.put(Index::ACCESS_METHOD,
             PQgetvalue(res, row_number,
                        static_cast<int>(OrdinalPosition::kAccessMethod)));

  // Set the boolean value converted to a string to property_tree.
  std::string is_unique = DbcUtils::convert_boolean_expression(PQgetvalue(
      res, row_number, static_cast<int>(OrdinalPosition::kIsUnique)));
  if (!is_unique.empty()) {
    object.put(Index::IS_UNIQUE, is_unique);
  }

  // Set the boolean value converted to a string to property_tree.
  std::string is_primary = DbcUtils::convert_boolean_expression(PQgetvalue(
      res, row_number, static_cast<int>(OrdinalPosition::kIsPrimary)));
  if (!is_primary.empty()) {
  ...
  object.put(Index::NUMBER_OF_KEY_COLUMNS,
             PQgetvalue(res, row_number,
                        static_cast<int>(OrdinalPosition::kNumKeyColumn)));
