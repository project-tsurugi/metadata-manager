...
ErrorCode ConstraintsDaoPg::select_all(
...
ErrorCode ConstraintsDaoPg::select(std::string_view key, std::string_view value,
    ...
    if (nrows >= 1) {
      if (key == Constraint::ID) {
        ...
        for (int row_number = 0; row_number < nrows; row_number++) {
          ptree constraint;
          // Convert acquired data to ptree type.
          constraint = convert_pgresult_to_ptree(res, row_number);
...
ErrorCode ConstraintsDaoPg::remove(std::string_view key, std::string_view value,
                                   ObjectId& object_id) const {
  ErrorCode error = ErrorCode::UNKNOWN;
