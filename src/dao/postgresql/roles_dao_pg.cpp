...
ErrorCode RolesDAO::convert_pgresult_to_ptree(
  ...
  role.put(
      Roles::ROLE_OID,
      PQgetvalue(res, row_number, static_cast<int>(OrdinalPosition::kOid)));
  ...
  role.put(
      Roles::ROLE_ROLCREATEROLE,
      DbcUtils::convert_boolean_expression(PQgetvalue(
          res, row_number, static_cast<int>(OrdinalPosition::kCreateRole))));

  // Set the value of the rolcreatedb column to ptree.
  role.put(Roles::ROLE_ROLCREATEDB,
           DbcUtils::convert_boolean_expression(PQgetvalue(
               res, row_number, static_cast<int>(OrdinalPosition::kCreateDb))));

  // Set the value of the rolcanlogin column to ptree.
  role.put(Roles::ROLE_ROLCANLOGIN,
           DbcUtils::convert_boolean_expression(PQgetvalue(
               res, row_number, static_cast<int>(OrdinalPosition::kCanLogin))));

  // Set the value of the rolreplication column to ptree.
  role.put(
      Roles::ROLE_ROLREPLICATION,
      DbcUtils::convert_boolean_expression(PQgetvalue(
          res, row_number, static_cast<int>(OrdinalPosition::kReplication))));

  // Set the value of the rolbypassrls column to ptree.
  role.put(
      Roles::ROLE_ROLBYPASSRLS,
      DbcUtils::convert_boolean_expression(PQgetvalue(
          res, row_number, static_cast<int>(OrdinalPosition::kBypassRls))));
